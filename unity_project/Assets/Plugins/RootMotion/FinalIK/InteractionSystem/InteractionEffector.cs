...
namespace RootMotion.FinalIK {
	...
	public class InteractionEffector {
		...
		private void PickUp(Transform root) {
            ...
            Debug.Log(interactionObject.transform.position);
            Debug.Log(effector.position);
			pickUpPosition = root.InverseTransformPoint(interactionObject.transform.position + new Vector3(0,0,0));
			...
			//interactionObject.transform.localPosition = -interactionObject.transform.localPosition;


			if (interactionSystem.OnInteractionPickUp != null) interactionSystem.OnInteractionPickUp(effectorType, interactionObject);
            if (interactionObject.targetsRoot.name.Contains("Clone")) {
                GameObject put_back = GameObject.Find(interactionObject.targetsRoot.name + "(Clone)");
                MeshRenderer[] arry_mr = put_back.GetComponentsInChildren<MeshRenderer>();
                foreach (MeshRenderer mr in arry_mr) {
                    mr.enabled = true;
                }
                GameObject to_delete = GameObject.Find(interactionObject.targetsRoot.name.Replace("(Clone)", ""));
                to_delete.SetActive(false);
                to_delete = GameObject.Find(interactionObject.targetsRoot.name);
                to_delete.SetActive(false);
                //UnityEngine.Object.Destroy(to_delete);
                put_back.name = put_back.name.Replace("(Clone)", "");
            } else {
                //sayonPickup(interactionObject.targetsRoot);
            }

            
        }

        void sayonPickup(Transform interactionObject) {
            string name = interactionObject.name.ToLower();
            //name = name.Replace("_", "");
            //name = Regex.Replace(name, "[0-9]", "");
            string[] candidates = { "{0}", "A/an {0}","Pick up {0}","Look at the {0}", "You have picked up a/an {0}.", "You are holding a/an {0} in your hand.", "That's a/an {0}." };
            Random rnd = new Random();
            int index = rnd.Next(candidates.Length);
            string output = String.Format(candidates[index], name);
            Debug.Log(output);
        ...
        public bool Stop() {
            ...
            Debug.Log(pickedUp);
            if (pickedUp) {
                //interactionObject.transform.position = target.position;
                //Debug.Log(interactionObject.transform.localPosition);
                GameObject go = GameObject.Find(interactionObject.targetsRoot.name);
