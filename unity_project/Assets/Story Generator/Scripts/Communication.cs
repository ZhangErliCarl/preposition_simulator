...
namespace StoryGenerator.Communication
{
    ...
    public class HttpCommunicationServer
    {
        ...
        public HttpCommunicationServer(int port)
        {
            ...
            httpServer.OnGet += GetEventHandler;
            httpServer.Stop();
            httpServer.Start();
        }
        ...
        public void GetEventHandler(object sender, HttpRequestEventArgs eventArgs) {
            Debug.Log("On Get");
            NetworkRequest networkRequest = JsonConvert.DeserializeObject<NetworkRequest>("{'action':'get_objects'}");
            Driver.SetRequest(networkRequest);
        }
