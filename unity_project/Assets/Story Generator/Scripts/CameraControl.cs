...
namespace StoryGenerator.Utilities
{
    ...
    public class AutoCameraControl : ICameraControl
    {
        ...
        public void ChangeCamera(Camera newCamera, bool doRefocus)
        {
            if (newCamera == null)
                return;
    ...
    public static class CameraUtils
    {
        ...
        public static bool ChangeCamera(IList<Camera> cameras, IList<int> indexes)
        {
            if (indexes.Count == 0)
            {
                return false;
            ...
            {
                if (index > cameras.Count - 1 || cameras[index] == null)
                    return false;
            }
            DeactivateCameras(cameras);
            
            switch (indexes.Count)
            {
                case 1:
                    cameras[indexes[0]].gameObject.SetActive(true);
                    cameras[indexes[0]].rect = new Rect(0, 0, 1, 1);

                    break;
                case 2:
                    cameras[indexes[0]].gameObject.SetActive(true);
                    cameras[indexes[0]].rect = new Rect(0, 0.5f, 1, 0.5f);
                    cameras[indexes[1]].gameObject.SetActive(true);
                    cameras[indexes[1]].rect = new Rect(0, 0, 1, 0.5f);
                    break;
                case 3:
                    cameras[indexes[0]].gameObject.SetActive(true);
                    cameras[indexes[0]].rect = new Rect(0, 0.5f, 0.5f, 0.5f);
                    cameras[indexes[1]].gameObject.SetActive(true);
                    cameras[indexes[1]].rect = new Rect(0.5f, 0.5f, 0.5f, 0.5f);
                    cameras[indexes[2]].gameObject.SetActive(true);
                    cameras[indexes[2]].rect = new Rect(0, 0, 0.5f, 0.5f);
                    break;
                case 4:
                    cameras[indexes[0]].gameObject.SetActive(true);
                    cameras[indexes[0]].rect = new Rect(0, 0.5f, 0.5f, 0.5f);
                    ...
                    cameras[indexes[2]].gameObject.SetActive(true);
                    cameras[indexes[2]].rect = new Rect(0, 0, 0.5f, 0.5f);
                    cameras[indexes[3]].gameObject.SetActive(true);
                    cameras[indexes[3]].rect = new Rect(0.5f, 0, 0.5f, 0.5f);
                    break;
                default:
                    return false;
